...
> Release candidate: `v2.3.15-rc2`。本项目承诺的是：对主题和基本用途可识别的正常请求，尽可能返回可编辑结果；不承诺任意本地模型都达到相同质量，也不代替人工审核。
...
发布仓库仅接受脱敏演示截图。请将不含客户资料、密钥、请求证据或本机路径的图片放入 [`docs/assets`](docs/assets/README.md)，然后在此处链接：

```text
docs/assets/demo-result.png（待维护者提供公开演示素材）
```
...
```bash
cp .env.example .env
# 编辑 .env，填写自己的本地模型地址和模型 ID
npm install
./scripts/check-environment.sh
./scripts/start.sh
```

macOS 也可以双击 `scripts/start-macos.command`。脚本从自身位置定位项目根目录，不会使用个人桌面路径、下载模型或改写第三方服务。示例 `.env` 使用 `3100`；以终端实际输出为准。更完整的中英文说明、provider 示例和排障步骤见 [docs/QUICK_START.md](docs/QUICK_START.md)。

## 通用本地模型配置

`.env.example` 是安全模板，复制为 `.env` 后再填写自己的值。不要提交 `.env`。
...
示例地址是 OpenWebUI 常见本地地址，不是产品默认绑定。请填写自己的运行时地址和实际模型 ID；如运行时不支持 JSON Schema，请把 `LOCAL_MODEL_SUPPORTS_JSON_SCHEMA` 改为 `false`。
